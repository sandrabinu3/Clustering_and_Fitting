...
def read_file_and_process_data(filename, selected_indicators):
    ...
    pivoted_data = df_2014.pivot(
        index='Country', columns='Indicator Name', values='Value')
...
def num_of_Clusters(scaled_df):
    """_Function to find the number of clusters using silhouette score_

    Parameters:
        scaled_df (_Dataframe_): _normalised dataframe
    """
...
def perform_kmeans_clustering(num_clusters, df):
    ...
    selected_countries = select_one_country_from_each_cluster(
        df, cluster_labels, countries)
...
def plot_clustered_data(df, cluster_labels, cluster_centers):
    ...
    ax.set_xlabel(
        'Electricity production from hydroelectric sources (% of total)', fontsize=12)
    ax.set_ylabel(
        'Electricity production from coal sources (% of total)', fontsize=12)
...
def select_one_country_from_each_cluster(df, cluster_labels, countries):
    ...
    for cluster in unique_clusters:
        ...
        selected_country = np.random.choice(
            df.loc[df.index[cluster_indices], 'Country'])
...
def predict_future_values(energy_use_data, countries, indicators, start_year, end_year, prediction_years):
    ...
    for country in countries:
        ...
        data = filter_energy_use_data(
            energy_use_data, country, indicators, start_year, end_year)
        ...
        for i in range(data.shape[0]):
            ...
            predicted_values = exponential_growth_function(
                extended_years - start_year, *popt)
            # standard deviation
            sigma = np.sqrt(np.diag(pcov))
            # lower and upper bounds
            low, up = err.err_ranges(
                extended_years-start_year, exponential_growth_function, popt, sigma)
...
if __name__ == '__main__':
    ...
    num_clusters = 3
    # perform clustering
    cluster_labels, cluster_centers = perform_kmeans_clustering(
        num_clusters, normalized_data)
    # plot the clustered data
    plot_clustered_data(
        normalized_data.iloc[:, 1:], cluster_labels, cluster_centers)
    # taking one country from each cluster
    countries_from_each_cluster = ['Morocco', 'Japan', 'Norway']
